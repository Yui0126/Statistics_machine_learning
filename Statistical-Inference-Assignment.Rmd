...
valueoj <- oj %>%
  summarise(mean = mean(len), median = median(len), min = min(len), max = max(len))
valuevc <- vc %>%
  summarise(mean = mean(len), median = median(len), min = min(len), max = max(len))
...
From the histogram below, we can see that those with `VC` have wider range of distribution, whereas that of `OJ` is more concentrated around the median. Also note that as the number of sample is not that high, distribution shape does not look that of a smooth normal distribution, but for the sake of analysis, I will use the data to conduct hypothesis testing in the next section. \newline From the boxplot, we can see that as the dose gets higher, the level of growth alos gets higher.
