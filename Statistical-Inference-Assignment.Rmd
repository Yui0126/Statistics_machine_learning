...
The dataset ["ToothGrowth"](https://www.rdocumentation.org/packages/datasets/versions/3.6.2/topics/ToothGrowth) contains information about the length of odontoblasts (cells responsible for the tooth growth) in 60 guinea pigs.\newline Results of the analysis are presented in the following order.

- **Sumamry Statistics of the dataset**. Summary statistics and distribution of the tooth length grouped by the delivery method (i.e. orange juice or ascorbic acid).
- **Hypothesis testing**. Evaluation of whether method and/or dose affect toothj growth through hypothesis testing.

Summary


### **Summary statistics of the tooth length grouped by delivery method**
From the table below, we can see that both mean and median of the tooth length of those who got orange juice (`OJ`) are higher than those who were given vitamin C (`VC`).
...
valueoj <- oj %>%
  summarise(mean = round(mean(len),3), median = median(len), min = min(len), max = max(len), sd = round(sd(len),3))
valuevc <- vc %>%
  summarise(mean = round(mean(len),3), median = median(len), min = min(len), max = max(len), sd = round(sd(len),3))
summary <- rbind(valueoj, valuevc)
rownames(summary) <- c("Orange Juice", "Vitamin C")
knitr::kable(summary, caption = "Summary statistics of the tooth length") %>%
  kable_styling(full_width = F, latex_options = "hold_position")
```

From the histogram below, we can see that those with `VC` have wider range of distribution, whereas that of `OJ` is more concentrated around the median. However, due to the small sample size it is difficult to conclude whether it has the shape of normal distribution. \newline From the boxplot, we can see that as the dose gets higher, the level of growth also gets higher.

```{r, fig.show='hold', out.width="45%",  fig.align='center', echo=FALSE}
...
So, could we actually say that different delivery method correlates with the average of the tooth length? Confidence interval was calculated manually, as well as using `t.test` function in `r` as shown in the table below. As the results contain $0$, we cannot conclude that the method is correlated with the tooth length.


```{r fig.show='hold', out.width="60%",  fig.align='center', echo=FALSE}
methodoj <- oj[,1]
methodvc <- vc[,1]
ci_t.test <- t.test(methodoj, methodvc, paired = FALSE)$conf
sp <- sqrt((29*6.606^2+29*8.266^2)/(30+30-2))
ci_manual <- 20.663-16.963+c(-1,1)*qt(.975,58)*sp*(1/30+1/30)^.5
df <- rbind(ci_t.test,ci_manual)
rownames(df) <- c("t.test","manual")
knitr::kable(df, caption = "Confidence Interval of methods") %>%
  kable_styling(full_width = F, latex_options = "hold_position")
```
