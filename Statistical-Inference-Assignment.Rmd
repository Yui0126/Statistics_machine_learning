...
### **Summary statistics of the tooth length grouped by vitamin C delivery method and dose**
...
### **Confidence Interval**
So, could we actually say that different delivery method and/or doses correlates with the average tooth length? Confidence intervals were calculated using `t.test` function as shown in the table below. \newline As for the value calculated the result contains $0$, we cannot conclude that the method is correlated with the tooth length. 
...
```
### **Hypothesis Testing**
We can also reach same conclusion if we conduct hypothesis test with the following hypotheses.  

- $H_0$: There is no difference in the mean between the two groups.  
- $H_a$: There is significant difference in the mean between the two groups.  

By setting the significance level $\alpha$ to be $0.05$ with two tails, we need the t-value to be more than $2$, but as shown in the t.test below, we can see that the value is less than $2$ and the p-value is also more than $0.05$.

```{r, out.width="60%",  fig.align='center', echo=FALSE}
t <- as.data.frame(round(t.test(methodoj, methodvc, paired = FALSE)$statistic,3))
p <- as.data.frame(round(t.test(methodoj, methodvc, paired = FALSE)$p.value,3))
table <- cbind(t,p)
...
dose1 <- dose_1[,1]
dose2 <- dose_2[,1]
t.test(dose1, dose0.5, paired = FALSE)$conf
